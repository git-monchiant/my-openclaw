...
def _ensure_tui_node() -> None:
    ...
    helper = PROJECT_ROOT / "dev" / "scripts" / "lib" / "node-bootstrap.sh"
...
def _launch_tui(
    ...
    tui_dir = PROJECT_ROOT / "dev" / "ui-tui"
...
def cmd_whatsapp(args):
    ...
    project_root = Path(__file__).resolve().parents[2]  # hermes_cli/ -> src/ -> repo root
    bridge_dir = project_root / "dev" / "scripts" / "whatsapp-bridge"
...
def _update_via_zip(args):
    ...
    _build_web_ui(PROJECT_ROOT / "dev" / "web")
...
def _update_node_dependencies() -> None:
    ...
    paths = (
        ...
        ("ui-tui", PROJECT_ROOT / "dev" / "ui-tui"),
    )
...
def cmd_dashboard(args):
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        if not _build_web_ui(PROJECT_ROOT / "dev" / "web", fatal=True):
            sys.exit(1)
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "src" / "hermes_cli" / "web_dist"
        )
