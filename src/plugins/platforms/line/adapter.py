...
class _LineClient:
    ...
    async def fetch_content(self, message_id: str) -> bytes:
        ...
        # TEST-ONLY hook: when LINE_TEST_IMAGE points to a local file, serve it
        # instead of calling LINE's content API — lets a synthetic webhook be
        # tested end-to-end with a real local image. No-op unless the env is set.
        _test_img = os.getenv("LINE_TEST_IMAGE", "").strip()
        if _test_img and os.path.exists(_test_img):
...
_QUICK_REPLY_RE = re.compile(r"\[\[\s*quickreply\s*:\s*([^\]]+?)\s*\]\]", re.IGNORECASE)


def _extract_quick_reply(content: str):
    """Pull a ``[[quickreply: A | B | C]]`` marker out of the agent's text and
    build a LINE quickReply object (tappable buttons that send their label as a
    message). Returns (cleaned_text, quick_reply_dict | None)."""
    m = _QUICK_REPLY_RE.search(content or "")
    if not m:
        return content, None
    labels = [x.strip() for x in m.group(1).split("|") if x.strip()][:13]  # LINE caps at 13
    cleaned = _QUICK_REPLY_RE.sub("", content or "").strip()
    items = [{"type": "action",
              "action": {"type": "message", "label": lbl[:20], "text": lbl}}
             for lbl in labels]
    return cleaned, ({"items": items} if items else None)
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _send_text_chunks(
        ...
        *,
        ...
        content, quick_reply = _extract_quick_reply(content)
        chunks = split_for_line(strip_markdown_preserving_urls(content))
        if not chunks:
            return SendResult(success=True, message_id=None)
        messages = [_text_message(c) for c in chunks][:LINE_MAX_MESSAGES_PER_CALL]
        if quick_reply and messages:
            # quickReply rides on the last bubble of the reply.
    ...
    # LINE loading API allows 5-60s per call. We request short bursts (15s)
    # so the bubble dismisses promptly when the bot replies — LINE's reply
    # dismissal can lag, and a long-duration request would leave the bubble
    # visibly hanging. Refresh every 12s while processing; cap at 5 rounds
    # (~60s of animation total) so a stuck agent can't keep it alive forever.
    _LINE_LOADING_SECONDS = 15
    _LINE_TYPING_REFRESH_SECONDS = 12.0
    _LINE_TYPING_MAX_ROUNDS = 5
    ...
    async def _keep_typing(
        ...
        **kwargs,
        ...
        """LINE-specific typing loop with bounded refresh + slow-LLM postback.

        Replaces the base 2s-refresh-forever loop with LINE-tailored cadence:
        the loading API serves 60s of animation per call, so we refresh
        every 50s and cap at 5 rounds (~4 minutes total) before letting
        the bubble lapse. The slow-response postback button still fires
        independently at its own threshold.
        """
        if not self._client or not chat_id:
            return

        # Slow-LLM postback button — runs in parallel with the typing loop.
        ...
        refresh = self._LINE_TYPING_REFRESH_SECONDS
        max_rounds = self._LINE_TYPING_MAX_ROUNDS
        rounds = 0
        try:
                ...
                if chat_id not in self._typing_paused:
                    try:
                        await asyncio.wait_for(
                            self._client.loading(
                                chat_id, seconds=self._LINE_LOADING_SECONDS
                            ),
                            timeout=5.0,
                        )
                        ...
                        logger.debug("LINE: loading error (non-fatal): %s", exc)
                rounds += 1
                if rounds >= max_rounds:
                    logger.info(
                        "LINE: typing animation cap reached (%d rounds) for chat %s — letting bubble lapse",
                        max_rounds, chat_id,
                    )
                    break
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        platform_hint=(
            ...
            "reply, so keep responses concise. NEVER send a separate "
            "acknowledgement such as 'ได้เลย รอสักครู่', 'กำลังทำ', or "
            "'กำลังวาด' before the result — the user already sees a loading "
            "animation while you work. Reply exactly ONCE, with the finished "
            "result. Image/audio/video sending "
            "requires LINE_PUBLIC_URL configured to a publicly reachable HTTPS "
            "host. To send a generated/local image, put ONLY the local file "
            "path returned by the image tool (its `image` field, e.g. "
            "/Users/.../cache/images/foo.png) on its own line — the gateway "
            "turns it into a real image. Do NOT use \\![](...) markdown and "
            "NEVER leave the parentheses empty; just output the bare path. "
            "Slow responses surface a 'Get answer' button the user taps "
