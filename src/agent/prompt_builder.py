...
def load_soul_md() -> Optional[str]:
    ...
    try:
        ...
        # family-bot patch: messaging-platform toolsets (LINE) have no terminal
        # tool, so the agent can't run `date` as <mandatory_tool_use> suggests.
        # Inject the current local time (config timezone) so "วันนี้/พรุ่งนี้"
        # and event times are answered in the family's timezone.
        try:
            from hermes_time import now as _hermes_now
            _now = _hermes_now()
            content += (
                "\n\n## Current date & time (authoritative — you cannot run `date`)\n"
                f"Now: {_now.strftime('%A %d %B %Y, %H:%M')} (Asia/Bangkok, UTC+7).\n"
                "Use THIS for 'วันนี้/พรุ่งนี้/เมื่อวาน'. ALWAYS convert event/match times "
                "to Thai time (UTC+7) before answering — never quote a foreign timezone "
                "like UTC-4 without converting."
            )
        except Exception as e:
            logger.debug("Could not append current time to SOUL.md: %s", e)
        return content
    except Exception as e:
