...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    # family-bot patch: do NOT reuse the stored system prompt. Upstream reuses
    # it verbatim so the Anthropic prefix cache hits, but that freezes BOTH the
    # injected current-time line (sessions drift days stale) AND the SOUL.md
    # content (rule edits never reach live sessions until /new). This install
    # runs Gemini at family-chat volume, so prefix-cache reuse is worthless —
    # always rebuild fresh. ``stored_prompt`` is still read above so the
