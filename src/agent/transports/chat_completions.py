...
def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    ...
    # Gemini 2.5 accepts thinkingBudget. Without one it thinks with a large
    # dynamic budget that can exhaust maxOutputTokens (thinking counts toward it)
    # and truncate the reply / multi-tool-call turns. Map the coarse effort to an
    # explicit budget so thinking stays bounded. (Pro accepts 128–32768.)
    if normalized_model.startswith("gemini-2.5-"):
        _budget = {"minimal": 256, "low": 1024, "medium": 4096,
                   "high": 12288, "xhigh": 24576}.get(effort, 4096)
