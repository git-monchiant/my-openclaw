...
def get_memory_dir() -> Path:
    """Return the memories directory, scoped per messaging user when available.

    On the gateway, ``HERMES_SESSION_USER_ID`` is set per inbound message (e.g.
    the LINE userId), so each person gets their own MEMORY.md / USER.md under
    ``memories/by_user/<id>`` — the family bot remembers each member separately.
    CLI / cron / any context without a session user fall back to the shared
    ``memories/`` root.
    """
