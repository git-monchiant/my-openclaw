...
IMAGE_GENERATE_SCHEMA = {
    ...
    "description": (
        ...
        "path in the `image` field. To deliver it, include that EXACT value "
        "(the full URL or absolute path) in your reply — easiest is to put it "
        "on its own line. The gateway extracts it and sends the real image. "
        "Never reference the image with empty parentheses like ![desc]() — "
        "that drops the path and the user gets no picture."
    ),
    "parameters": {
        "type": "object",
        "properties": {
