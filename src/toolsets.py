...
TOOLSETS = {
    ...
    # ── family-bot tools (Classroom / Trello / Calendar) — see FAMILY.md ──
    # Native tools registered by tools/{classroom,trello,calendar}_tool.py.
    "classroom": {
        "description": "Google Classroom data (assignments, status, summary) for family-bot",
        "tools": ["classroom"],
        "includes": []
    },

    "trello": {
        "description": "Trello board management (view/create/move/archive cards) for family-bot",
        "tools": ["trello"],
        "includes": []
    },

    "calendar": {
        "description": "Google Calendar schedules (tutoring, football, ...) for family-bot",
        "tools": ["calendar"],
        "includes": []
    },
    # ── end family-bot tools ──
    ...
    "hermes-line": {
        # NOTE: requires gemini-2.5-pro (or another reliable tool-caller).
        # gemini-2.5-flash intermittently returns an EMPTY completion (no
        # content, no tool call) once tools are attached — verified flaky on
        # flash, rock-solid on pro. If you switch the model back to flash,
        # expect dropped replies on LINE.
        "description": "LINE platform default toolset (family-bot / TT^ homework bot)",
        # family tools: classroom, trello, calendar (see FAMILY.md) + image gen + core helpers.
        # NOTE: `clarify` is intentionally excluded — on a chat platform it blocks
        # the turn for minutes waiting for a reply (left the bot "spinning"); TT^
        # just asks via a normal message (+ quick-reply buttons) instead.
        # fetch_page replaces web_extract: every web_extract backend needs a paid
        # API key (none configured), so it always errored; fetch_page is the
        # keyless full-page reader (see FAMILY.md).
        "tools": ["classroom", "trello", "calendar", "image_generate", "memory", "session_search", "web_search", "fetch_page", "worldcup"],
        "includes": []
    }
}
